package by.it.group551004.gavrilovroman.lesson1;
...
public class FiboC {
    ...
    long fasterC(long n, int m) {
        ...
        return countNthFibMod((int)(n % pi), m);
    }
    ...
    private int countPiOfPrime(int p) {
        if (p == 2) return 3;
        if (p == 5) return 20;
    ...
    private void countPi() {
        ...
        pi = (int)Math.pow(p, k - 1) * countPiOfPrime(p);

        for(int i = 1; i < primes.size(); i++) {
            p = primes.get(i);
            k = powers.get(i);
            pi = findLcm(pi, (int)Math.pow(p, k - 1) * countPiOfPrime(p));
        }
    }

    private int countNthFibMod(int n, int m) {
        if (n == 0 || n == 1) return 1;
