package by.it.group551002.liashkevich.lesson3;
...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        for (Map.Entry<Character, Integer> bububu : count.entrySet()) {
            char childSymb = bububu.getKey();
            int childFreq = bububu.getValue();
            LeafNode pupupu = new LeafNode(childFreq, childSymb);
            priorityQueue.add(pupupu);
        }
        ...
        while (priorityQueue.size() > 1) {
            ...
            InternalNode lululu = new InternalNode(first, second);
            priorityQueue.add(lululu);
        }
        //4. последний из родителей будет корнем этого дерева
        //это будет последний и единственный элемент оставшийся в очереди priorityQueue.
        Node kukuku = priorityQueue.poll();
        kukuku.fillCodes("");
        StringBuilder sb = new StringBuilder();
        for (char c : s.toCharArray()) {
            String finSymb = codes.get(c);
            sb.append(finSymb);
        }
